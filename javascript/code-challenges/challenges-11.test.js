...
function transformToLis(obj){
 ...
 for(let i = 0; i < array.length; i++){
  for(let j = 0; j < array[i].length-1; j ++){
    listed.push(`<li>${array[i][j]}: ${array[i][j+1]}</li>`)
  }
 }
 return listed;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let returnArr = input.map(item => item.filter(element => element%5 === 0))
  returnArr = returnArr.map(item => item.filter(element => typeof(element) === 'number'))
  return returnArr.map(item => item.map(element => Math.pow(2, element)))
...
let findShortest = (data) => {
  ...
  let shortest = "";
  data.map(item => {
    if(Number(item.height) < short){
      short = Number(item.height);
      shortest = item.name;
    }
  }
    )
