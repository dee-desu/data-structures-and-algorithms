...
const sortStarWarsCharacters = (starWarsArr) => {
  // Solution code here...
  return starWarsArr.sort( (x,y) => y.height - x.height);
};
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(i => {
    let index = i.indexOf(' ',3);
    let forDel = i.length - index - 1
    let ingredient = i.slice(-forDel)
    result.push(ingredient);
  })
  result.reduce( (arr, insertion) => {
    return arr.concat(insertion)
  }, [])
  return result;
};
