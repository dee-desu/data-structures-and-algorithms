...
const addValues = (arr) => {
  // Solution code here...
  const add = arr.reduce((previousValue, currentValue) => previousValue+currentValue,0);
  return add;
};
...
const addPurchases = (arr) => {
  // Solution code here...
  let total = arr.reduce((aacu, val) => {
    return aacu + val.purchasePrice
  }, 0)
  return(total)
};
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  return(arr.reduce((lastVal, val, index) => {
    lastVal + val
    return index+1}
    ));
};
...
const returnNames = (arr) => {
  // Solution code here...
  const names = [];
    arr.reduce((lastVal, val) => {
        names.push(val.name)
    })
    names.unshift(starWarsData[0].name);
    return names;
};
...
const reversedString = (str) => {
  // Solution code here...
  return (str.split("").reduce((reverse, string)=> string + reverse, ''));
};
...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  const childrenCount = arr.map(item => {
    return item.children
 })
 
 const finalArr = [];
 for (let i = 0; i<childrenCount.length; i++){
   if(childrenCount[i] !== undefined){
    finalArr.push(childrenCount[i]);
   }
 }
 return finalArr.join().split("-").reduce(sum => sum + 1, 0)
};
