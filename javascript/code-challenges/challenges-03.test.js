...
const addTwo = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++){
    let adddTwoN = arr[i] + 2;
    newArr.push(adddTwoN);
  }
  return newArr;
};
...
const typeNum = (arr) => {
  // Solution code here...
  const filtered = arr.filter(index => !isNaN(index))
  return filtered;
};
...
const containsAnd = (arr) => {
  // Solution code here...
  const andArr = arr.filter(item => item.includes("and"));
  return andArr;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  const notIncludedArr = arr.filter( item => !(forbiddenValues.includes(item)))
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  // Solution code here...
  const statArr = arr.filter(item => item.baseStat > minBaseStat)
  return statArr;
};
...
const getStatName = (arr, minBaseStat) => {
  // Solution code here...
  const StatName = getBaseStatGreaterThan(arr, minBaseStat).map( item => item.stat.name)
    return StatName;
};
...
const getCharactersWithoutChildren = (arr) => {
  // Solution code here...
  const lannistersSuck = arr.filter(item => {
    if(item.children === undefined){
        return true;
    }
})
return lannistersSuck
};
